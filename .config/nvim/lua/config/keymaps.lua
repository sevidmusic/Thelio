...
-- Go to next buffer | Use lazy vim's keybinding instead: <SHIFT>-L and <SHIFT>-H
-- vim.keymap.set("n", "<leader>nb", ":bnext<cr>", { noremap = true, silent = true })

-- Go to previous buffer | Use lazy vim's keybinding instead: <SHIFT>-L and <SHIFT>-H
...
-- NEED TO INSTALL showMethods from MyLinux into .darling/bin
...
-- snake to camel (whole line)
vim.keymap.set("n", "<leader>stc", ":s/_\\([a-zA-Z]\\+\\)/\\u\\1/g<cr>", { noremap = true, silent = true })

-- kebab to camel (whole line)
vim.keymap.set("n", "<leader>ktc", ":s/-\\([a-zA-Z]\\+\\)/\\u\\1/g<cr>", { noremap = true, silent = true })

-- camel to snake (whole line)
vim.keymap.set(
  "n",
  "<leader>cts",
  ":s/\\<\\([a-z]\\+\\)\\(\\u\\+\\)/\\l\\1_\\2/g<cr>0v$u",
  { noremap = true, silent = true }
)

-- camel to kebab (whole line)
vim.keymap.set(
  "n",
  "<leader>ctk",
  ":s/\\<\\([a-z]\\+\\)\\(\\u\\+\\)/\\l\\1-\\2/g<cr>0v$u",
  { noremap = true, silent = true }
)
